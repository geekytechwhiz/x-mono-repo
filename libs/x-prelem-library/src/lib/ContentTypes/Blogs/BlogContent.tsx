...
function BlogContent(props: any) {
  ...
  const { getEventUrl = "", secondaryArgs = {} } = props;
  ...
  const handleContentTypeNavigate = (item: any) => {
    const { item_path = [] } = item;
    if (nullToArray(item_path)?.[0]?.content_type) {
      const type = conCatUrlPath(item_path[0]?.content_type);
                    ...
                    <TimelineContent sx={{ display: "flex", padding: 0 }}>
                      ...
                      <Box
                        ...
                        {item?.authors[0] ? (
                          ...
                          <Typography
                            onClick={() => handleContentTypeNavigate(item)}
                            variant='p4regular'
                            component='p'
                            className='description'
                            sx={{
