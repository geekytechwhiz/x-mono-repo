...
import { ReplyIcon, capitalizeFirstLetter, useUserSession } from "@platformx/utilities";
...
const CommentPreview: React.FC<PreviewProps> = ({ parentRef }: PreviewProps) => {
  ...
  const findCommentId = (elementId: any, action = "after") => {
    ...
    return groupedComments[Object.keys(groupedComments)[action === "prev" ? id - 1 : id + 1]][0];
  };
  useEffect(() => {
    setOpen(
          ...
          <>
                  ...
                  <IconButton
                    ...
                    onClick={(event: any) => {
                      ...
                      Number(selectedComment.elementId) - 1 > 0 &&
                        (document
                          .getElementById(
                            findCommentId(Number(selectedComment.elementId).toString(), "prev")
                              .elementId,
                          )
                          ?.scrollIntoView({
                            behavior: "smooth",
                    ...
                    <NavigateBeforeOutlinedIcon sx={{ color: "rgba(0, 0, 0, 0.3)" }} />
                  </IconButton>
                  <IconButton
                    aria-label='next'
                    onClick={(event: any) => {
                      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
                      Number(selectedComment.elementId) + 1 <= comments.length &&
                        (document
                          .getElementById(
                            findCommentId(Number(selectedComment.elementId).toString()).elementId,
                          )
                          ?.scrollIntoView({
                            behavior: "smooth",
                            block: "start",
                        ...
                        setTimeout(() => {
                          handleCommentClick(
                            event,
                            findCommentId(Number(selectedComment.elementId).toString()).elementId,
                            findCommentId(Number(selectedComment.elementId).toString()).commentId,
                          );
                        }, 500));
                    }}>
                    <NavigateNextOutlinedIcon sx={{ color: "rgba(0, 0, 0, 0.3)" }} />
                  </IconButton>
                </Box>
                <Box>
                  <Tooltip title={selectedComment.isResolved ? "Resolved" : "Resolve"}>
                    <IconButton aria-label='settings' sx={{ padding: "6px" }}>
                      <TaskAltIcon
                        ...
                        sx={{
                          color: selectedComment.isResolved ? "green" : "rgba(0, 0, 0, 0.5)",
                          width: "20px",
                          height: "20px",
                        }}
                      />
            ...
            {!enableEdit && (
              <>
                ...
                {selectedComment?.reply?.map((selectedCommentItem, index) => (
                  <>
                    {" "}
                    <Box display='flex' flexDirection='row' justifyContent='space-between'>
                      <Box display='flex' alignItems='center'>
                        <Tooltip
                          title={selectedCommentItem?.reviewer}
                          placement='left-end'
                          sx={{ cursor: "pointer" }}>
                          <span style={{ cursor: "pointer" }}>
                            <Avatar
                              ...
                              {...stringAvatar(selectedCommentItem?.reviewer)}
                            />
                          </span>
                        </Tooltip>
                        <Typography
                          ...
                          {capitalizeFirstLetter(
                            selectedCommentItem?.reviewer, //userInfo.first_name
                          )}
                        </Typography>
                        <Typography
                          variant='h7semibold'
                          ...
                          {formatTimestamp(selectedCommentItem?.timeStamp)}
                        </Typography>
                      </Box>
                      {/* <Box>
                      <IconButton
            ...
            {enableEdit && selectedId === selectedComment.commentId && (
              <CommentEditor comment={selectedComment} onCancel={setEnableEdit}></CommentEditor>
            )}
          </>
        }
