import { Typography, ListItem, FormControlLabel, Checkbox } from "@mui/material";
        ...
        <Box className={classes.listContainer}>
          ...
          {renderList.length > defaultAmountOfItem && (
            <Typography
              onClick={() => {
                setAmountOfItem(
                  amountOfItem === renderList.length ? defaultAmountOfItem : renderList.length,
                );
              }}
              variant='h7regular'
              className={classes.typoViewMoreOrLess}>
              {amountOfItem === renderList.length ? t("view_less") : t("view_more")}
            </Typography>
          )}
        </Box>
      </Box>
