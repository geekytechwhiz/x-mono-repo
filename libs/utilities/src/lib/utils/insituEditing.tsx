import { handleHtmlTags } from "./helperFns";
...
export const doneInsituEditing = (keys, ref, buttonsKeysPopulatedObj, prelemId?: any) => {
  ...
  for (let i = 0; i < updatedEditableList?.length; i++) {
    // HTML Node
    const htmlNode = ref?.current?.querySelector(`#${updatedEditableList[i]?.id}`);
    if (htmlNode) {
      // Reference of every element
      const field = updatedEditableList[i];
      let IsParagraph = false;
      if (prelemId === "Prelem_007" || prelemId === "Prelem_023" || prelemId === "Prelem_026") {
      ...
      for (const validationKey in validations) {
        switch (validationKey) {
          case "maxlength":
            errors.push({
              [validationKey]: isMaxLength(field, validations[validationKey], IsParagraph),
            });
            break;
          case "required":
            errors.push({
      ...
      if (htmlNode instanceof HTMLButtonElement) {
        field.type = "button";
        field.buttonKeysObject = buttonsKeysPopulatedObj[updatedEditableList[i]?.id];
      }
    }
  }
  if (!updatedEditableList.some((key) => key.errorString && key.errorString !== "")) {
    for (let i = 0; i < updatedEditableList?.length; i++) {
