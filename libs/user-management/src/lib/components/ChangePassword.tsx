...
import { PLATFORMXLOGO } from "@platformx/utilities";
import { putRequest } from "@platformx/authoring-apis";
import { resetState } from "@platformx/authoring-state";
...
const ChangePassword = () => {
  ...
  const [changePasswordSuccess, setChangePasswordSuccess] = React.useState<boolean>(false);
  const dispatch = useDispatch();

  const handleSubmit = () => {
    const clone = { ...errors };
    if (data.currentPassword.length < 8) clone.currentPassword = "Minimum 8 characters";
    ...
    if (data.newPassword.length < 8) clone.newPassword = "Minimum 8 characters";
      ...
      clone.newPassword = "New password must be different from current password.";
    else clone.newPassword = "";
    if (data.confirmPassword.length < 8) clone.confirmPassword = "Minimum 8 characters";
  ...
  const handleChange = (prop: keyof State) => (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const clone = { ...errors };
    if (prop === "currentPassword" && data.newPassword !== event.target.value) {
      clone.newPassword = "";
    }
    if (prop === "newPassword" && data.confirmPassword === event.target.value) {
      clone.confirmPassword = "";
    }
    if (prop === "confirmPassword" && data.newPassword === event.target.value) {
      clone.newPassword = "";
    }
    ...
    <Container maxWidth='md'>
      <Box
        ...
        <Box
          ...
          <img src={PLATFORMXLOGO} height='30' alt='' />
        </Box>
        <Paper
          sx={{
            maxWidth: 500,
        ...
        <Box
          ...
          onClick={handleBack}>
          <ArrowBackIosNew sx={{ color: "#0077B5" }} fontSize='small' />
          <Typography
