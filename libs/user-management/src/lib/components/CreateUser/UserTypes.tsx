...
export default function UserTypes({ t, state, setState, setRoleSelected, isEditMode }: any) {
    ...
    if ((event.target as HTMLInputElement).value === t("end_user")) {
      setState((prevState: any) => {
        return {
          ...prevState,
          role: "",
          is_Authoring_User: (event.target as HTMLInputElement).value === t("authoring_user"),
          is_Rendering_User: (event.target as HTMLInputElement).value === t("end_user"),
        };
      });
      setRoleSelected("");
    } else {
        ...
        <Grid container>
          ...
          <Grid item xs={12} sm={7} md={7} lg={7} className='textFiledLast'>
            <RadioGroup
              ...
              value={state.is_Authoring_User ? t("authoring_user") : t("end_user")}
