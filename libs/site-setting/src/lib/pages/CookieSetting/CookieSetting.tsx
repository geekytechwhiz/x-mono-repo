...
export const CookieSetting = () => {
  ...
  const multiSelectChange = (e, name) => {
    setConsentList((preState) =>
      preState.includes(name) ? preState.filter((c) => c !== name) : [...preState, name],
    );
  };
  const initForm = () => {
  ...
  const handleInputChange = useCallback((event) => {
    ...
    const control = form.find((ctrl) => name === ctrl.name);
    if (!control) return;
    control.value = value;
    setForm([...form]);
  }, []);
            ...
            <Box id='informativecookiesetting' ref={informativeFormRef}>
              ...
              >
                <Box className={classes.informativeContentContainer}>
                  <Grid container>
                    <Grid item xs={12} md={8} sm={8} lg={8} className={classes.informativeLeft}>
                      <Grid container rowSpacing={1}>
                        {renderListControlFromArr(form.slice(0, 5))}
                        <Grid item xs={12} sm={12} md={12} className={classes.leftGridItem}>
                          <TitleSubTitle
                        ...
                        <Grid item xs={12} sm={12} md={12} className={classes.rightGridItem}>
                          <MultiSelect
                            containerStyle={{
                    ...
                    <Grid item xs={12} md={4} sm={4} lg={4} className={classes.informativeSkeleton}>
