...
const InstructorDashBoard = () => {
  ...
  useEffect(() => {
    dashBoard?.coursesList?.length !== 0 && setCourses(courseListMapper(dashBoard?.coursesList));
  }, [dashBoard]);
  ...
  return (
    <Box className={classes.container}>
      ...
      <Typography variant='h6medium' mt='5px'>
        ...
        {overDueTaskLength() > 0 && `${t("and")} ${overDueTaskLength()} ${t("overdue_task_text")}`}
      </Typography>
      {/* Page And Content section */}
      <Box className={classes.sectionMargin}>
        <Grid container>
          <Grid item xs={12} md={12} em={12} lg={12} sx={{ marginTop: { xs: "20px", lg: "0" } }}>
