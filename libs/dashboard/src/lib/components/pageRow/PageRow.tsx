...
const PageRow = ({
  ...
  const handleClick = (status: string) => {
    if (status == "published") {
      ...
      fetchPageModel(dispatch, runFetchPageModel, runFetchValidationQuery, path, navigate);
    }
  };
    ...
    <Box className={classes.contentRowContainer}>
      <Grid container>
        ...
        <Grid
          ...
          <Box className={status === "published" ? classes.statusPublish : classes.statusDraft}>
            {`${t(status)}`}
          </Box>
        </Grid>
        <Grid
          ...
          <Box className={classes.arrowIcon} onClick={() => handleClick(status)}>
