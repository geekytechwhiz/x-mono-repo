...
export const VodMenu = (props) => {
  const { anchorEl, open, handleClose, listItemDetails, category, subCategory } = props;

  const { canAccessAction } = useAccess();
  const classes = useStyles();
  const { dispatch } = useContext(Store);
  const { deleteVod, unPublish, onCopy, handleOpenVod, fetchContentDetails, duplicate } = useVod();

  const [vodSettingsPanelState, setVodSettingsPanelState] = useState<boolean>(false);
  ...
  const [confirmImageOrVideoDelete, setConfirmImageOrVideoDelete] = useState<boolean>(false);
  ...
  const handlePublishedPageView = async (vodPayload) => {
    ...
    if (page_state === "published") {
      window.open(`${authInfo.publishUri + i18n.language}/` + `video${currentPageUrl}`);
  ...
  const getDuplicateTitle = () => {
    ...
    const duplicateVodTitle = newVal.length > 100 ? newVal.slice(0, 100) : newVal;
    ...
    <>
      ...
      <Menu
        ...
        <MenuItem
          ...
          <VisibilityIcon /> {listItemDetails.page_state == "published" ? t("view") : t("preview")}
        </MenuItem>
        ...
        <ErrorTooltip
          component={
            <MenuItem
              style={{
                cursor: listItemDetails.page_state === "pending" ? "not-allowed" : "pointer",
              }}
              disableRipple
        ...
        (listItemDetails.page_state == "draft" && listItemDetails.lastPublishedDate) ? (
          <MenuItem
            ...
            <LinkRoundedIcon sx={{ transform: "rotate(-45deg)" }} /> {t("copy_url")}
          </MenuItem>
        ) : null}
        <MenuItem
          disableRipple
      ...
      {galleryState && (
        <Box className={classes.galleryContainer}>
          ...
          <DamContentGallery
            handleImageSelected={handleSelectedImage}
            toggleGallery={toggleGallery}
            assetType={galleryType === "Images" ? "Image" : "Video"}
