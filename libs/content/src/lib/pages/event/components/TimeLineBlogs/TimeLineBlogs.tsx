...
import { updateBlogApiCall, defaultFallBackImage } from "./Utils/helperBlogs";
...
const TimeLineBlogs = () => {
  ...
  const commonSaveData = (resArray: any = []) => {
    ...
    if (
      nullToArray(resArray[0]?.item_path).length > 0 &&
      resArray[0]?.item_path[0]?.item_Details?.page
    ) {
      ...
      const newObj = {
        ...
        path: nullToString(item?.page),
        EditorialItemPath: nullToString(item?.page),
  ...
  const publishBlog = async () => {
    ...
    const descriptionData = !blogData?.BlogAuthorName
      ? `<span style="word-wrap: break-word" class="onlydesc">${
          nullToString(blogData?.BlogTextArea) + nullToString(mediaUrl)
        } </span>`
  ...
  return (
    <>
      <style>{styles}</style>
      <Grid className='liveBlogger' container sx={{ backgroundColor: "#f5f6f8" }}>
        <Grid
          item
          xs={12}
          lg={12}
        ...
        <TabContext value={selectedTabvalue}>
          ...
          <Grid container className='test-tab' sx={{ display: { xs: "unset" } }}>
            <TabPanel
              ...
              <Grid item xs={12} sx={{ borderRadius: "4px" }} className='platx-event'>
