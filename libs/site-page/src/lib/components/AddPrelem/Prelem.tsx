...
import { RootState, updateContentForCard } from "@platformx/authoring-state";
...
import { AUTH_INFO } from "../../../../../utilities/src";
import ButtonEditLink from "../ButtonEditLink/ButtonEditLink";
import ButtonEditWindow from "../ButtonEditWindow";
import { PrelemActions } from "../utils/constants";
...
const Prelem = ({
  ...
  const getStructuredData = (strDataOld, dataIndex, isPrelemDataReset) => {
    dispatch(
      updateContentForCard({
        ...
        sectionToUpdate: "StructuredData",
        data: strDataOld,
        index: undefined,
      }),
    );
  ...
  function logKey(e) {
    e.code === "Space" || e.code === "Enter" ? setSpaceKey(true) : setSpaceKey(false);
  }
  ...
  useEffect(() => {
    ...
    const buttonProps = Object.keys(page?.prelemMetaArray?.[index].content)?.reduce(
      (acc: any, item: any) => {
        ...
        if (item?.includes("RedirectURL")) {
          acc = {
            ...acc,
            [`${item}_Internal`]:
              page?.prelemMetaArray?.[index].content?.[item.replace("RedirectURL", "Action")] ===
              "Internal"
  ...
  const updateStateValuesForButton = (buttonId) => {
    const selectedWindowValue = currentButtonObject.current[`${buttonId.replace("Name", "Type")}`];
    const selectedPageUrl =
      currentButtonObject.current[`${buttonId.replace("Name", "RedirectURL_Internal")}`];
    const selectedExternalLink =
      currentButtonObject.current[`${buttonId.replace("Name", "RedirectURL_External")}`];

    const selectedAction = currentButtonObject.current[`${buttonId.replace("Name", "Action")}`];
    const selectedContentLink =
      convertToLowerCase(selectedAction) !== convertToLowerCase("Content")
        ? currentButtonObject.current[`${buttonId.replace("Name", "RedirectURL_Content")}`]
        : currentButtonObject.current[`${buttonId.replace("Name", "Content")}`];
    const jsVal =
      selectedAction === "Js"
        ? currentButtonObject.current[`${buttonId.replace("Name", "RedirectURL")}`]
        : "";
  ...
  const handleEditLinkButtonRef = (buttonRef, e) => {
    ...
    if (
      !(
        ...
        refButtonEditableCurrentTargetState.current[e.currentTarget.id]?.target.id ===
          e.currentTarget.id
      )
    ) {
      refButtonEditableCurrentTargetState.current[e.currentTarget.id] = {
    ...
    refCurrentEditableButtonEditWindowValues.current.buttonTarget = e.currentTarget;
    const { clientX } = e;
    const { clientY } = e;
    const viewPortHeight: any = typeof window !== "undefined" ? window.innerHeight : "";
    const topVal: any = viewPortHeight - clientY > 200 ? clientY + 40 : clientY + -60;
    if (!spaceKey) {
    ...
    sendDefaultStructureDataForResetToAuthoringCB: (strDataNew = PrelemId) => {
      dispatch(
        updateContentForCard({
          ...
          sectionToUpdate: "DefaultStructureDataForReset",
          data: strDataNew,
          index: undefined,
        }),
      );
  ...
  const handleEdit = (action) => {
    const validationKeysArray =
      page?.prelemsValidationObject[page?.prelemMetaArray?.[index].DocumentType];
    console.info(
      "validation check",
    ...
    console.info("Prelem Keys", prelemKeys);
  ...
  const updateCurrentButtonObject = (value, buttonId, actionType) => {
    ...
    if (actionType === "RedirectURL_Content") {
      currentButtonObject.current[`${buttonId.replace("Name", "Content")}`] = value; //content data append
    }
    updateStateValuesForButton(buttonId);
  };

  const handleWindowOptionChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUrlInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    refCurrentEditableButtonEditWindowValues.current.externalWindowLink = event.target.value;
  ...
  const handleUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (refCurrentEditableButtonEditWindowValues.current.buttonAction === "Internal") {
      setExternalLink(refCurrentEditableButtonEditWindowValues.current.pageWindowUrl);
    } else if (refCurrentEditableButtonEditWindowValues.current.buttonAction === "Content") {
      setExternalLink(refCurrentEditableButtonEditWindowValues.current.contentUrl);
    }
    // else if (
    ...
    // }
      ...
      setExternalLink(refCurrentEditableButtonEditWindowValues.current.externalWindowLink);
    }
    refButtonEditableCurrentTargetState.current[
      refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
    ] = {
  ...
  const createButtonState = (buttonRef, buttonStateTarget) => {
    ...
    if (
      ...
      buttonState[buttonRef.Button_RedirectURL] =
        refButtonEditableCurrentTargetState.current[
          refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
        ]?.currentEditWindow?.pageWindowUrl || prelemState?.[buttonRef.Button_RedirectURL];
    } else if (
      refButtonEditableCurrentTargetState.current[
        refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
      ...
      buttonState[buttonRef.Button_RedirectURL] =
        refButtonEditableCurrentTargetState.current[
          refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
        ]?.currentEditWindow?.contentUrl || prelemState?.[buttonRef.Button_RedirectURL];
    } else {
      buttonState[buttonRef.Button_RedirectURL] =
        refButtonEditableCurrentTargetState.current[
          refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
        ]?.currentEditWindow?.externalWindowLink || prelemState?.[buttonRef.Button_RedirectURL];
    }
    buttonState[buttonRef.Button_Action] =
      refButtonEditableCurrentTargetState.current[
        refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
      ]?.currentEditWindow?.buttonAction || prelemState?.[buttonRef.Button_Action];
    buttonState[buttonRef.Button_Type] = refButtonEditableCurrentTargetState.current[
      refCurrentEditableButtonEditWindowValues.current.buttonTarget.id
  ...
  const handleEditButtonWindowRef = (clientX, clientY) => {
    const viewPortHeight: any = typeof window !== "undefined" ? window.innerHeight : "";
    const topVal: any = viewPortHeight - clientY > 180 ? clientY + 20 : clientY + -280;
    setEditButtonWindowPostion({
      left: clientX + -200,
  ...
  const constructButtonObjForSubmit = () => {
    const buttonsList = Object.keys(currentButtonObject.current).filter((item) =>
      item.includes("Name"),
    );
    const buttonProps = buttonsList.reduce((acc, item) => {
      const selectedAction = currentButtonObject.current[`${item.replace("Name", "Action")}`];
      let selectedUrl = "";
      if (selectedAction === "Internal")
        selectedUrl =
          currentButtonObject.current[`${item.replace("Name", "RedirectURL_Internal")}`];
      else if (selectedAction === "External")
        selectedUrl =
          currentButtonObject.current[`${item.replace("Name", "RedirectURL_External")}`];
      else if (selectedAction === "Js")
        selectedUrl = currentButtonObject.current[`${item.replace("Name", "RedirectURL")}`];
      else if (selectedAction === "Content")
        selectedUrl = currentButtonObject.current[`${item.replace("Name", "RedirectURL_Content")}`];
      const ContentData = currentButtonObject.current[`${item.replace("Name", "Content")}`];
      acc = {
        ...acc,
        [item]: {
    ...
    <Box
      ...
      <Box
        ...
        <Box
          ...
          <ul>
            {iconsSet
              .filter((item) => item.id !== (showIconsState.showVisible ? "show" : "hide"))
              .map((value, key) => {
                return (
                  <PrelemButtons
                    key={key}
        ...
        <CommentWrapper
          ...
          <Box
            sx={{
              ...
              pointerEvents: `${
                showIconsState.showVisible || (!prelemEditState && isHover) ? "none" : ""
              }`,
            }}>
        ...
        {showAddSection.showAtTop ? (
          <Box component='div' className={classes.topaddCtaWp} style={showOnHover}>
            {addSectionTouchPoint(index, "top")}
          </Box>
        ) : null}
        {showAddSection.showAtBottom ? (
          <Box component='div' className={classes.bottomaddCtaWp} style={showOnHover}>
