...
import CardList from "../PrelemCardLayout/CardList";
import PrelemList from "../PrelemList/PrelemList";
...
const SearchPrelem = () => {
  ...
  const [inputValue, setInputValue] = React.useState<any>(
    url.searchParams.get("inputValue") ? (url.searchParams.get("inputValue") as string) : "",
  );
  ...
  useEffect(() => {
    ...
    if (inputValue === "") {
      if (url.searchParams.get("searchText") || url.searchParams.get("inputValue")) {
  ...
  const handleCategoryFilter = (name: string, index: any) => {
    ...
    if (!categoryNewIndexArr.includes(index.toString())) {
      ...
      categoryNewIndexArr.splice(categoryNewIndexArr.indexOf(index.toString()), 1);
    }
