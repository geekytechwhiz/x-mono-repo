...
const AdvancedSeo = ({ setPageId }) => {
  ...
  const tagsPrefilled = !page?.pageModel.Others ? "" : JSON.parse(page?.pageModel.Others);
  const {
    noindex,
    nofollow,
    nosnippet,
  ...
  const data = [
    ...
    {
      tagState: tagsPrefilled !== "" ? tagsPrefilled["max-image-preview"] : false,
    ...
    {
      tagState: tagsPrefilled !== "" ? tagsPrefilled["max-video-preview"] : false,
  ...
  const handleTagCheckChange = (e, index) => {
    ...
    if (tagsUpdated[index].tagName === "max-snippet" && tagsUpdated[index].tagState === false) {
  ...
  const saveAdvancedSeo = () => {
    ...
    dispatch(updatePageModel({ pageModelCreated: pageModelNew }));
    const pageDataObj = {
              ...
              <Box key={index}>
                <Typography className='labelbox' variant='p4regular'>
                  <FormControlLabel
                    ...
                    label={<Typography variant='p4regular'>{t(tag.id)}</Typography>}
                  />
                </Typography>
