...
import { getData, postData } from "@platformx/utilities";
...
export const usePollApi = () => {
  ...
  const getPoll = async (publicRuntimeConfig, pageData) => {
    try {
      ...
      const data = await getData(
        `${process.env.NX_API_URL}poll/fetch?document_path=${pageData.document_path}`,
      );
  ...
  const onSubmit = async (pollData, publicRuntimeConfig, pageData) => {
    try {
      ...
      const dataResp = await postData(`${process.env.NX_API_URL}poll/save`, pollToSend);
      setLoading(false);
